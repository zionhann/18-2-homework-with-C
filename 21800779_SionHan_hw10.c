...
void memSearch(MEMBERS mem[])
{
    ...
    inputName[strlen(inputName)-1]='\0'; //remove '\n'
    
    for(int i=0;i<g_count;i++) {
        if(!strcasecmp(mem[i].name, inputName)) //incasesensitive
        {
            putchar('\n');
            printf("Name         : %s              \n", mem[i].name);
    ...
    if(g_count!=20)
    {
        do{
            do{
                ...
                }while(checkDgt(mem)); //ID must be 6 characters
            
            capID(mem); //Capitalize the ID
        
            }while(checkDup(mem)); //to avoid duplication of ID
            
        printf("Enter the name: ");
        lineFeed=getchar();
        fgets(mem[g_count].name, sizeof(mem[g_count].name), stdin);
        capName(mem); //Capitalize the name
